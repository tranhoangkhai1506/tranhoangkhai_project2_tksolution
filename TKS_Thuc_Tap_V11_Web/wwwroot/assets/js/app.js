...
(function () {
	...
	var horizontalMenuSplit = 13; // after this number all horizontal menus will be moved in More menu options
	...
	function windowLoadContent() {
		...
		if (hide_hover_menu != 1) {
			if (document.getElementById("tks_navbar")) {
				document.getElementById("tks_navbar").addEventListener("mousemove", Show_Menu_Hover);
			}

			if (document.getElementById("tks_navbar")) {
				document.getElementById("tks_navbar").addEventListener("mouseleave", Hide_Menu_Hover);
			}
		}
	...
	function addEventListenerOnSmHoverMenu() {
		if (document.getElementById("vertical-hover") != null) {
	...
	//Handle click event on topbar menu
	var currPopup = null;
	var activePopup = null;
	function isShowing(el) {
		if (el && $(el).children().last().css('display') == 'block') {
			return true;
		}
		return false;
	}
	...
	function hidePopup(el) {
		if (isShowing(el)) {
			...
			$(el).children().first().prop("aria-expanded", false);
		}
	}
	function hideAllPopup() {
		var mainMenu = $('[data-layout=horizontal] .navbar-nav > .nav-item');
		for (var i = 0; i < mainMenu.length; i++) {
			hidePopup(mainMenu[i]);
		}
	}
	function isMainMenu(el) {
		var obj = $(el).parents('.main-menu');
		return (obj.length || $(el).hasClass('main-menu'));
	}
	function highlightMenu(el) {
		$(el).children().first().prop("aria-expanded", true);
	}
	$('[data-layout=horizontal] .navbar-nav > .nav-item').on('click', function (e) {
		if (isMainMenu(e.target)) {
			currPopup = this;
			if (isShowing(currPopup)) {
				hideAllPopup();
			}
			else {
				hideAllPopup();
	...
	$(document).on('click', function (e) {
		var elem = $('[data-layout=horizontal] .navbar-nav .nav-item');
		if ($(e.target).closest(elem).length === 0) {
			hideAllPopup();
			highlightMenu(activePopup);
		}
